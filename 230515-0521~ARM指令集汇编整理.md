...
FIQ有更多的独立寄存器，在存储现场压栈时压入内存的寄存器更少以此达到更快的中断，同时FIQ在中断向量表的最末尾，FIQ操作的代码段则紧接这中断向量表，从而少跳转一次达到更快的中断效果。
...
指令语句有对应的机器码，伪指令没有对应的机器码。伪指令的格式与[指令的格式](# 指令格式)相同。
...
这种操作可以在压栈中使用，xA为空栈式压栈，xB为满栈式压栈，这在之后的软中断中会再次提到。
